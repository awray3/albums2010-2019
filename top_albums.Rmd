...
# create boolean for whether or not an album shows up on multiple lists
aotd_wide$n_lists <- rowSums(aotd_wide[-(1:2)]!=0)

aotd_wide_multiple_reviewers <- (
  aotd_wide 
  %>% dplyr::filter(n_lists>1)
  %>% select(-n_lists, -Time)
)

aotd_wide_multiple_reviewers_ranks <- aotd_wide_multiple_reviewers
aotd_wide_multiple_reviewers_ranks[-(1:2)] <-
  (
    aotd_wide_multiple_reviewers_ranks[-(1:2)]
    %>% lapply(FUN= function(x) ifelse(x!=0, 201-x, NA))
  )

hovertext <- apply(aotd_wide_multiple_reviewers_ranks,
                   MARGIN = 1,
                   FUN= function(row) paste(paste0("</br>", colnames(aotd_wide_multiple_reviewers_ranks)), row, sep=": ", collapse = " ") )

rev_pca_multiple_reviewers <- prcomp(aotd_wide_multiple_reviewers %>% select(-album, -artist), scale=TRUE )
p <- ggbiplot(rev_pca_multiple_reviewers, scale=0) + 
  geom_point(aes(text=hovertext))
ggplotly(p, tooltip="text")
  #  autoplot(rev_pca_multiple_reviewers, loadings=TRUE, loadings.label=TRUE) +
```
